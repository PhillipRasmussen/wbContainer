...
<cfif structkeyexists(url,"rule_action") and url.rule_action EQ 'move' AND structKeyExists(url,'ruleIDs')>
	<!--- see if current list is greater than new list --->
	<cfif listLen(url.ruleIDs) GT arraylen(stConObj.aRules)>
		<cfloop list="#url.ruleIDs#" index="i" >
			<cfif NOT arrayFind(stConObj.aRules,i)>
				<cfquery  datasource="#application.dsn#" name="qTmp">
				SELECT * FROM container_arules
				WHERE data = '#i#'									
				</cfquery>
				<cfset stOldConObj = application.fapi.getContentObject(objectid="#qTmp.parentid#",typename="container")>
				<cfset stOldConObj.aRules.delete(i) />
				<cfset setData(stProperties=stOldConObj) />
			</cfif>
		</cfloop>
	</cfif>
	<!--- loop over list --->
	<cfset aNewRules = []>
	<cfloop list="#url.ruleIDs#" index="i" >
		<cfset aNewRules.append(i)>
	</cfloop> 
	<cfset stConObj.aRules = aNewRules />
	<cfset setData(stProperties=stConObj) />
	<skin:bubble title="Container management" tags="container,update,info"><cfoutput>The rule has been moved</cfoutput></skin:bubble>
</cfif>
	...
	<cfif structkeyexists(url,"rule_action") and structkeyexists(url,"rule_id") and structkeyexists(url,"rule_index") and url.rule_index lte arraylen(stConObj.aRules)>
		...
		<cfswitch expression="#url.rule_action#">
			...
			<cfcase value="move">
				<!--- use this to move and Rule to another container or reorder current container --->
			</cfcase>
		</cfswitch>
...
<cfelseif request.mode.design and request.mode.showcontainers gt 0>
	<cfoutput><div class="containerContent" data-objectid="#stConObj.objectid#"></div></cfoutput>
</cfif>
